...
TEST_JWT_SECRET = "test-jwt-secret-endpoints-v2-32chars"
...
def get_admin_token():
    """Get an admin JWT token by logging in with test credentials."""
    with patch.multiple(
...
def test_new_routes():
    ...
    with settings_patch:
        ...
        res = client.post("/api/events/create", json=evt_payload, headers=auth_headers)
        ...
        index_res = client.post("/api/admin/index-faces", json={"event_id": event_id}, headers=auth_headers)
        ...
        with zipfile.ZipFile(_io.BytesIO(zip_res.content), "r") as zf:
