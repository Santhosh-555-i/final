...
/**
 * Normalizes photo image URLs to ensure clean paths pointing to live backend static CDN.
 */
...
export async function createEvent(
  ...
  const res = await fetch(`${getApiBaseUrl()}/events/create`, {
    method: "POST",
...
export async function searchFaceApi(
  ...
  const compressedSelfie = await compressImage(selfieFile, 720, 0.85);
...
export async function recomputeClusters(eventId: string): Promise<PersonCluster[]> {
  const res = await fetch(`${getApiBaseUrl()}/clusters/event/${encodeURIComponent(eventId)}/recompute`, {
    method: "POST",
  });
  if (!res.ok) {
...
export async function renamePersonCluster(clusterId: string, name: string): Promise<void> {
  const res = await fetch(`${getApiBaseUrl()}/clusters/${clusterId}/rename`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function mergePersonClusters(sourceClusterId: string, targetClusterId: string): Promise<void> {
...
export async function deletePersonBiometrics(clusterId: string): Promise<void> {
  const res = await fetch(`${getApiBaseUrl()}/clusters/${clusterId}`, {
    method: "DELETE",
  });
  if (!res.ok) {
...
export async function createShareToken(eventId: string, photoIds: string[], expiryHours = 24): Promise<{ token: string; share_url: string; expires_at: string }> {
  const res = await fetch(`${getApiBaseUrl()}/sharing/create-token`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      event_id: eventId,
...
export async function getSharedPhotosByToken(token: string): Promise<{ event_id: string; event_title: string; event_code: string; photos: PhotoData[]; expires_at: string; is_revoked: boolean }> {
  const res = await fetch(`${getApiBaseUrl()}/sharing/view/${encodeURIComponent(token)}`);
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
...
export async function getEventAuditLogs(eventId: string): Promise<AuditLogEntry[]> {
  const res = await fetch(`${getApiBaseUrl()}/sharing/audit-logs/${encodeURIComponent(eventId)}`);
  if (!res.ok) {
    return [];
  }
  const data = await res.json();
...
export async function deleteEventBiometrics(eventId: string): Promise<void> {
  const res = await fetch(`${getApiBaseUrl()}/sharing/event/${encodeURIComponent(eventId)}/biometrics`, {
    method: "DELETE",
  });
  if (!res.ok) {
...
export async function getEventSettings(eventId: string): Promise<EventSettingsData> {
  const res = await fetch(`${getApiBaseUrl()}/sharing/settings/${encodeURIComponent(eventId)}`);
  if (!res.ok) {
    return {
      event_id: eventId,
